...
export default async () => {
    Bloggify.ready(async err => {
        ...
        for (let [groupName, group] of Object.entries(actions)) {
            if (!group) { continue; }
            const actionGroup = {
            ...
            delete actionGroup.default

            for (let [name, action] of Object.entries(actionGroup)) {
                ...
                if (actionGroup.before) {
                    Bloggify.actions[method](actionName, async ctx => {
                        await actionGroup.before(ctx, name, actionName)
                        return action(ctx)
                    }, htmlCb)
                } else {
                    Bloggify.actions[method](actionName, async ctx => {
